...
public interface IItemFluidContainerV1 extends IItemFluidContainer
{
	...
	{
		return stack.getCapability(V.CAPABILITY_FLUID, null).fill(resource, doFill);
	}
	
	default FluidStack drain(ItemStack stack, int maxDrain, boolean doDrain)
	{
		return stack.getCapability(V.CAPABILITY_FLUID, null).drain(maxDrain, doDrain);
	}
	
	default FluidStack drain(ItemStack stack, FluidStack resource, boolean doDrain)
	{
		return stack.getCapability(V.CAPABILITY_FLUID, null).drain(resource, doDrain);
	}
