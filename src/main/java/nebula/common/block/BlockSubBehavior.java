...
public abstract class BlockSubBehavior extends BlockBase implements IUpdateDelayBlock, ISmartFallableBlock, IToolableBlock, IThermalCustomBehaviorBlock
{
			...
			ItemStack stack)
	{
		try
		{
			getBehavior(state).onBlockPlacedBy(this, state, worldIn, pos, placer,
					placer.getHorizontalFacing(), stack);
		}
		catch (Exception exception)
		{
			NebulaLog.catching(exception);
	...
	public boolean isOpaqueCube(IBlockState state)
	{
		...
		catch (Exception exception)
		{
			...
			{
				/**
				 * Called when block is initialize, the data is not exist yet.
				 * If there are better way to solve this problem: TODO.
				 */
				return true;
			}
			NebulaLog.catching(exception);
