...
import nebula.common.nbt.INBTSelfReader;
import nebula.common.nbt.INBTSelfWriter;
...
public class ItemBase extends Item implements IRegisteredNameable, IRenderRegister
{
	...
	{
		NebulaLog.info("Reloading items...");
		for (ItemBase item : list)
		{
			item.postInitalizedItems();
		}
		NebulaLog.info("Reloaded {} items.", list.size());
		list = null;
	}
	...
	protected ItemBase(String modid, String name, String unlocalizedTooltip, String localTooltip)
	{
		...
		NebulaRegistry.registerItem(this, name);
		/**
	...
	{
		return LanguageManager.translateLocal(getTranslateName(stack), getTranslateObject(stack));
	}
	...
	protected class CapabilityProvider implements ICapabilityProvider, INBTSerializable<NBTTagCompound>
	{
		...
		{
			if (this.provider instanceof INBTSelfWriter<?>)
				return ((INBTSelfWriter<NBTTagCompound>) this.provider).writeTo();
			else
		...
		{
			if (this.provider instanceof INBTSelfReader<?>)
				((INBTCompoundReader) this.provider).readFrom(nbt);
			else
				this.nbt = nbt;
		}
