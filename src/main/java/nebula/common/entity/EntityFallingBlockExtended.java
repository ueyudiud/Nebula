...
public class EntityFallingBlockExtended extends Entity
{
	...
	public static void replaceFallingBlock(World world, BlockPos pos, IBlockState state, int height)
	{
		...
		if (!hited.getBlock().isAir(hited, world, pos))
		{
			...
			{
				W.spawnDropsInWorld(world, pos, hited.getBlock().getDrops(world, pos, hited, 0));
			}
		}
	}
	...
	public EntityFallingBlockExtended(World world, BlockPos pos, BlockPos pos1, IBlockState state, TileEntity tile, boolean flammable)
	{
		...
		{
			NebulaLog.error("Fail to create a new falling block, disable this falling action.", exception);
			setDead();
