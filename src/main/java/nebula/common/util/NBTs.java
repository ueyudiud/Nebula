...
public final class NBTs
{
	...
	{
		writer.writeTo(compound, key, obj);
	}
	...
	public static <E> E[] getArrayOrDefault(NBTTagCompound nbt, String key, E[] container, INBTCompoundReader<E> reader)
	{
		if (nbt.hasKey(key, NBT.TAG_LIST))
		{
			...
			for (int i = 0; i < list.tagCount(); ++i)
			{
				...
				try
				{
					container[id] = reader.readFrom(nbt1);
				}
				catch (Exception exception)
				{
					NebulaLog.catching(exception);
	...
	{
		return reader.readFrom(nbt, key);
	}
	...
	public static <E, N extends NBTBase> INBTReaderAndWriter<E[], NBTTagList> wrapAsUnorderedArrayWriterAndReader(INBTReaderAndWriter<E, N> rw)
	{
		final E[] instance = ObjectArrays.newArray(rw.type(), 0);
		return new INBTReaderAndWriter<E[], NBTTagList>()
		{
			@Override
			public E[] readFrom(NBTTagList nbt)
			{
				E[] result = ObjectArrays.newArray(instance, nbt.tagCount());
				for (int i = 0; i < nbt.tagCount(); ++i)
				{
					result[i] = rw.readFrom((N) nbt.get(i));
				}
				return result;
			}
			
			@Override
			public NBTTagList writeTo(E[] target)
			{
				NBTTagList list = new NBTTagList();
				for (E element : target)
				{
					list.appendTag(rw.writeTo(element));
				}
				return list;
			}
			...
			public Class<E[]> type()
			{
				return (Class<E[]>) instance.getClass();
			}
		};
	}
}
