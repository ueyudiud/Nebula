...
public final class W
{
	...
	private static final Method isOutsideBuildHeight;
	
	static
	{
		try
		{
			Method method = World.class.getDeclaredMethod("isOutsideBuildHeight", BlockPos.class);
			method.setAccessible(true);
			isOutsideBuildHeight = method;
		}
		catch (Throwable t)
		{
	 ...
	 */
	public static void spawnDropsInWorld(World world, BlockPos pos, @Nullable Iterable<ItemStack> drops)
	{
	 ...
	 */
	public static void spawnDropsInWorld(ICoord coord, Iterable<ItemStack> drop)
	{
	 ...
	 *         otherwise.
	 * @see W#fallBlock(World, BlockPos, BlockPos, IBlockState)
	 */
	...
	{
		return NebulaProxy.proxy.worldInstance(dimID);
	}
	...
	public static boolean isOutsideBuildHeight(World world, BlockPos pos)
	{
		try
		{
			return (boolean) isOutsideBuildHeight.invoke(world, pos);
		}
		catch (Throwable t)
		{
			throw new InternalError(t);
	...
	/**
	 * Set block to {@link V#AIR}
	 * @param world the setting world.
	 * @param pos the pos to delete block.
	 * @return
	 */
	public static boolean setAirBlock(World world, BlockPos pos)
	{
		if (isOutsideBuildHeight(world, pos))
		{
			return false;
		}
		else if (!world.isRemote && world.getWorldInfo().getTerrainType() == WorldType.DEBUG_WORLD)
		{
			return false;
		}
		else
		{
			...
			{
				blockSnapshot = BlockSnapshot.getBlockSnapshot(world, pos, 3);
				world.capturedBlockSnapshots.add(blockSnapshot);
			}
			IBlockState oldState = chunk.getBlockState(pos);
			
			IBlockState replacedState = setAirBlock(world, chunk, pos, oldState);
			...
			{
				if (oldState.getLightOpacity(world, pos) != 0/*|| oldState.getLightValue(world, pos) != 0*/)
				{
				...
				{
					world.markAndNotifyBlock(pos, chunk, replacedState, V.AIR, world.isRemote ? 11 : 3);
				}
				return true;
			}
		}
	...
	/**
	 * Set block with specific BlockState and TileEntity.
	 * @param world the world.
	 * @param pos the block position.
	 * @param state the block state.
	 * @param te the tile entity.
	 * @param flags the refresh flags.
	 * @return return <code>true</code> if block is set.
	 */
	public static boolean setBlock(World world, BlockPos pos, IBlockState state, TileEntity te, int flags)
	{
		if (isOutsideBuildHeight(world, pos))
		{
			return false;
		...
		else
		{
			...
			IBlockState replacedState = setBlock(world, chunk, pos, state, te, oldState);
			...
			{
				if (state.getLightOpacity(world, pos) != oldOpacity || state.getLightValue(world, pos) != oldLight)
				{
					world.theProfiler.startSection("checkLight");
					world.checkLight(pos);
					world.theProfiler.endSection();
				...
				if (blockSnapshot == null) // Don't notify clients or update physics while capturing blockstates
				{
					world.markAndNotifyBlock(pos, chunk, replacedState, state, flags);
				}
				return true;
			}
		}
	...
	private static IBlockState setAirBlock(World world, Chunk chunk, BlockPos pos, IBlockState state2)
	{
		int i = pos.getX() & 15;
		int j = pos.getY();
		int k = pos.getZ() & 15;
		...
		{
			int k1 = state2.getLightOpacity(world, pos); // Relocate old light value lookup here, so that it is called before TE is removed.
			ExtendedBlockStorage storage = chunk.storageArrays[j >> 4];
			...
			if (k1 > 0 || chunk.getLightFor(EnumSkyBlock.SKY, pos) > 0 || chunk.getLightFor(EnumSkyBlock.BLOCK, pos) > 0)
			{
	...
	private static IBlockState setBlock(World world, Chunk chunk, BlockPos pos, IBlockState state, TileEntity tile, IBlockState state2)
	{
		int i = pos.getX() & 15;
		int j = pos.getY();
		int k = pos.getZ() & 15;
		...
		else
		{
			...
			if (storage == Chunk.NULL_BLOCK_STORAGE)
			{
				...
				{
					storage = new ExtendedBlockStorage(j >> 4 << 4, !world.provider.hasNoSky());
					chunk.storageArrays[j >> 4] = storage;
					flag = j >= i1;
				}
			}
			else
			...
			storage.set(i, j & 15, k, state);
			
			if (!world.isRemote)
			{
				state2.getBlock().breakBlock(world, pos, state2);
			}
			chunk.removeTileEntity(pos);
