 ...
 * {@link nebula.asm.MutableIntEntry}.
 * <p>
 * 
...
public class HashIntMap<K> extends AbstractIntMap<K>
{
	...
	int									size;
	int									sum;
	transient INode<IntEntry_<K>>[]	entries;
	...
	public HashIntMap(Map<? extends K, ? extends Number> map)
	{
		...
		{
			putUnchecked(new IntEntry_<K>(entry.getKey(), entry.getValue().intValue()));
	...
	public HashIntMap(IntMap<? extends K> map)
	{
		...
		this.sum = map.sum();
		this.size = map.size();
		for (IntEntry<? extends K> entry : map)
		{
			putUnchecked(new IntEntry_<K>(entry.getKey(), entry.getValue()));
		}
	}
	...
	private Integer putChecked(K target, int value)
	{
		...
		{
			INode<IntEntry_<K>>[] entries = this.entries;
			this.entries = new INode[(this.size + 1) + (this.size >> 1)];
			for (INode<IntEntry_<K>> node : entries)
			{
				if (node == null) continue;
				for (IntEntry_<K> entry : node)
				{
					putUnchecked(entry);
				}
			}
		}
		return putUnchecked(new IntEntry_<>(target, value));
	}
	
	private Integer putUnchecked(IntEntry_<K> entry)
	{
		...
		{
			for (IntEntry_<K> entry1 : this.entries[i])
			{
	...
	private IntEntry_<K> getEntry(Object key)
	{
		...
		else
		{
			try
			{
				for (IntEntry_<K> entry : this.entries[i])
				{
	...
	{
		IntEntry_<K> entry = getEntry(key);
	...
	public void rescale() throws IllegalStateException
	{
		...
		Iterator<IntEntry_<K>> iterator = (Iterator<IntEntry_<K>>) (Iterator) iterator();
		...
		for (IntEntry_<? extends K> entry : (Iterable<IntEntry_<? extends K>>) (Iterable) this)
			entry.value /= i;
	}
	...
	private int removeAt(int id, INode<IntEntry_<K>> node)
	{
	...
	public int remove(Object key)
	{
		...
		{
			INode<IntEntry_<K>> node0 = INode.telomereNode(this.entries[i]);
			do
			{
				node0 = node0.next();
	...
	@Override
	public Iterator<IntEntry<K>> iterator()
	{
	...
	@Override
	public int computeIfAbsent(K key, ToIntFunction<? super K> function)
	{
		IntEntry_<K> entry = getEntry(key);
		if (entry != null)
		{
		...
		{
			int result;
			put(key, result = function.applyAsInt(key));
			return result;
		}
	}
	...
	class IntegerMapItr implements Iterator<IntEntry<K>>
	{
		boolean					modified;
		int						pointer	= -1;
		INode<IntEntry_<K>>		currentNode;
		...
		public void remove()
		{
			...
			INode<IntEntry_<K>> old = this.currentNode;
	...
	class IntegerMapSet extends AbstractSet<K>
	{
		...
		{
			return Iterators.transform(HashIntMap.this.iterator(), IntEntry::getKey);
		}
