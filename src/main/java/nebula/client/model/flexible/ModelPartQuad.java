...
public class ModelPartQuad implements INebulaModelPart
{
	...
	public static final JsonDeserializer<INebulaModelPart> LOADER = (json, typeOfT, context) -> {
		...
		part.tintindex = J.getOrDefault(object, "tint", -1);
		part.renderTwoFace = J.getOrDefault(object, "biface", true);
		JsonElement json1 = object.get("pos");
		if (json1.isJsonArray())
		{
			if (json1.getAsJsonArray().get(0).isJsonArray())// Full position  data.
			{//TODO
				part.pos = Jsons.getArray(json1.getAsJsonArray(), 4, float[].class, J::getFloat3);
			}
			else// x, y, z, w, h
			{
				float[] datas1 = J.getFloat5(json1);
		...
		if (object.has("uv"))
		{
			float[] datas = J.getFloat4(object.getAsJsonArray("uv"));
		...
		part.icon = J.getOrDefault(object, "texture", "#all");
		return part;
	};
