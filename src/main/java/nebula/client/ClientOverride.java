...
public class ClientOverride
{
	...
	public static List<BakedQuad> renderItemModel(IBakedModel model, EnumFacing facing, long rand, ItemStack stack)
	{
		...
		catch (RuntimeException exception)
		{
			if (model != null)
			{
				V.error("Invalid model {} by stack {}, failed to get facing at {}.", model, stack, facing);
				V.catching(exception);
			}
			else
			{
				V.error("No model present by {}!", stack);
			}
	...
	public static ItemStack renderItemOverlay(RenderItem render, FontRenderer fr, ItemStack stack, int xPosition, int yPosition, @Nullable String text)
	{
		...
		if (item == null)
		{
			V.error("The rendering item is missing. ItemRender : {}, FontRenderer : {}", render, fr);
			return null;
		}
		try
		{
		...
		{
			V.error("Failed to render item overlay into GUI. Item : {}, ItemRender : {}, FontRenderer : {}", stack, render, fr);
			V.catching(exception);
			return null;
		}
	}
	
	public static void onMinecraftStartLoading()
	{
		NebulaClientHandler.onMinecraftStartLoading();
	}
}
