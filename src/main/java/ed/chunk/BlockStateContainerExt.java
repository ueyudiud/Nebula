...
public class BlockStateContainerExt extends BlockStateContainer
{
	...
	protected void setBit(int size)
	{
		if (this.bits != size)
		{
			...
			this.palette.idFor(V.AIR);// Mark air id to 0
			this.bits = size;
		...
		for (int i = 0; i < 16; ++i)
			for (int j = 0; j < 16; ++j)
				for (int k = 0; k < 16; ++k)
				{
					try
					{
						set(k, i, j, datas == null ? V.AIR : ExtendedBlockStateRegister.INSTANCE.getStateFromData(datas[id]));
					}
					catch (Exception exception)
					{
						ED.logger.catching(exception);
					}
					++id;
				}
	}
